...
import SuccessPopup from './SuccessPopup'; // 새 SuccessPopup 컴포넌트 가져오기
...
const ConfirmPopup: React.FC<ConfirmPopupProps> = ({ onClose, onConfirm }) => {
  const [showSuccess, setShowSuccess] = useState(false); // 성공 팝업을 처리하기 위한 상태

  const handleConfirm = () => {
    onConfirm(); // onConfirm 핸들러 호출
    setShowSuccess(true); // 성공 팝업을 표시
  };

  const handleCloseSuccessPopup = () => {
    setShowSuccess(false); // 성공 팝업을 닫기
    onClose(); // 확인 팝업을 닫기
  };
